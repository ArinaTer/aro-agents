...
function getSortedMediaElements() {
  if (COMMON_MEDIA_QUERIES.MOBILE) {
    duplicateSlides('.about__swiper-mob .about__swiper-thumbs .swiper-wrapper');
    duplicateSlides('.about__swiper-mob .about__swiper-main .swiper-wrapper');
  } else {
    duplicateSlides('.about__swiper-desktop .about__swiper-thumbs .swiper-wrapper');
    duplicateSlides('.about__swiper-desktop .about__swiper-main .swiper-wrapper');
  }
...
function getAnimatePreloaderFn() {
  ...
  const preloaderTotal = document.querySelector(".preloader-percent span");
  const preloaderLogo = document.querySelector(".preloader-logo_loading");
  const preloaderLineLogo = document.querySelector(".preloader-line-loading");

  if (preloaderTotal) {
    return function animatePreloader(value) {
      let path = value * 100;
      preloaderTotal.innerHTML = Math.floor(value * 100);
      preloaderLogo.style.width = Math.floor(value * 100) + '%';
      preloaderLineLogo.style.width = Math.floor(value * 100) + '%';
    };
  } else {
    return function animatePreloader(value) {
    };
